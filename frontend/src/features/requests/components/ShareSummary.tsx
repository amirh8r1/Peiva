...
interface ShareSummaryProps {
  ...
  /** لیبل بج — در پنل مشارکت‌کننده «سهم شما از تولید» */
  label?: string;
}

/** پنل سهم مشارکت‌کننده — شفافیت کامل: درصد، کیلوگرم، ارزش‌ها. */
export function ShareSummary({ share, label = 'سهم مشارکت‌کننده از تولید' }: ShareSummaryProps) {
  return (
    <ProfitShareBadge percent={share.supplierSharePercent} label={label}>
      <Space direction="vertical" size={4} style={{ width: '100%', marginTop: 8 }}>
        ...
        <SummaryRow label="ارزش نهاده مشارکت‌کننده" value={`${formatNumber(share.supplierInputCost)} تومان`} />
      </Space>
    </ProfitShareBadge>
  );
}
